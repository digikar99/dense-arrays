                     ...
                     (cond ((typep ss 'int-index)
                           ...
                           ((listp ss)
                            (destructuring-bind (&optional (start 0)
                              ...
                              (setq start (normalize-index start d))
                              (unless (= end d)
                                (setq end (normalize-index end d)))
...
(def-test aref* (:suite backend-independent)
  (symbol-macrolet ((array (make-array '(10 2) :constructor #'+ :element-type 'int32)))
    ...
    (is (equalp '(0 2) (array-dimensions (aref* array '(0 :end 0)))))
    (symbol-macrolet ((array (make-array 5 :initial-contents '(1 2 3 4 5) :element-type 'int32)))
      (is (array= (make-array 5 :initial-contents '(5 4 3 2 1) :element-type 'int32)
                  (aref* array '(-1 :step -1))))
      (is (array= (make-array 4 :initial-contents '(5 4 3 2) :element-type 'int32)
                  (aref* array '(-1 :end 0 :step -1)))))
